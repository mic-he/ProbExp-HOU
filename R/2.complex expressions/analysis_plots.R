...
j.samples=jags(data=dataList,
               ...
               n.burnin=2500, n.thin=1, DIC=F) # burn in, thinning values, compute DIC
...
for (i in 1:length(samples[,1])){
  ...
  # speaker
  df.speaker.pred=predictions.sp(alpha,beta,lambda,theta.might,theta.probably,theta.certainly)$speaker
  # listener
...
expression.bars.ci=ggplot(data=df.e)+
  ...
  theme_bw()+
  theme(strip.background = element_blank(),text=element_text(size=26),legend.position="none",panel.spacing.x=unit(1, "lines"))
...
expression.vs.ppc.ci=ggplot(data=df.e)+
  ...
  ylab("data")+
  theme_bw()+
  theme(strip.background = element_blank(),text=element_text(size=26),legend.position="bottom")
...
state.bars.ci=ggplot(data=df.s)+
  ...
  theme_bw()+
  theme(strip.background = element_blank(),text=element_text(size=24),
...
# long plot with three interpretation plots together
bars.long.ci=grid.arrange(state.bars.ci,access.bars.ci,observation.bars.ci, ncol = 3,
                          left=textGrob("choice count", gp=gpar(fontsize=24), rot = 90, just = "centre")) 
show(bars.long.ci)
# three vs plots together
vs.long.ppc.ci=grid.arrange(state.vs.ppc.ci,access.vs.ppc.ci,observation.vs.ppc.ci, ncol = 3,
                            left=textGrob("data", gp=gpar(fontsize=24), rot = 90, just = "centre"),
                            bottom=textGrob("prediction", gp=gpar(fontsize=24), rot = 0, just = "centre")) 
show(vs.long.ppc.ci)
